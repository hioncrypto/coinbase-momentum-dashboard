# Coinbase Momentum & Volume Dashboard — CLOUD v5.5
# - Auto tries Exchange endpoints first, then Advanced
# - First-tick tolerant subscribe logic (+ fallback)
# - Hard per-attempt timeout (ensures we fall through)
# - Safe config overrides (no programmatic writes to st.session_state)
# - Restart stream, Top Movers, RSI/EMA, alert panel, diagnostics
...
st.set_page_config(page_title="Momentum & Volume — CLOUD v5.5", layout="wide")
...
st.markdown("# Momentum & Volume Dashboard — **CLOUD v5.5**")
st.caption("Auto prefers Exchange, hard timeouts, tolerant subscribe, restart, safe config, movers, alerts, diagnostics.")
...
async def subscribe_and_expect_first_tick(ws, mode, channel, product_ids, chunk):
    ...
    Tolerates a non-ticker first message (e.g. 'subscriptions' or 'hello'),
    then keeps reading briefly to find a real ticker.
    """
    def chunks(lst, n):
    ...
    # Wait up to FIRST_TICK_TIMEOUT for any message,
    # then allow +2s more to find a real ticker if the first isn't one.
...
async def ws_once(url, mode, channel, products, chunk):
    ...
    Adds a hard 12s cap so we *always* fall through to the next URL.
    """
    import websockets
