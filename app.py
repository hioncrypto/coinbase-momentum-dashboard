...
<style>
:root {
    --hc-sidebar-width: 340px;
}

/* Sidebar as fixed, drag-resizable overlay */
section[data-testid="stSidebar"] {
    position: fixed !important;
    top: 0;
    left: 0;
    height: 100vh;
    width: var(--hc-sidebar-width) !important;
...
/* Sticky top controls area inside sidebar */
...
/* Main app view always full viewport width */
...
/* Keep sidebar toggle arrow fixed at FAR LEFT */
...
def find_pivots(close: pd.Series, span: int = 3) -> Tuple[List[int], List[int]]:
    ...
    for i in range(span, n - span):
        if (values[i] > values[i - span:i].max()
                and values[i] > values[i + 1:i + 1 + span].max()):
            highs.append(i)
        if (values[i] < values[i - span:i].min()
...
with st.sidebar:
    ...
    if st.session_state.get("use_my_pairs", False):
        avail_pairs = [
            p.strip().upper() for p in st.session_state.get("my_pairs", "").split(",") if p.strip()
        ]
    elif st.session_state.get("use_watch", False):
        avail_pairs = [
            p.strip().upper() for p in st.session_state.get("watchlist", "").split(",") if p.strip()
        ]
    else:
        effective_exchange = (
            "Coinbase"
...
with expander("Gates"):
    ...
    with c1:
        ...
        if st.session_state.get("use_vol_spike"):
            new_vm = st.slider(  # RANGE CHANGED TO 20
                "Spike multiple",
                1.0,
                20.0,
