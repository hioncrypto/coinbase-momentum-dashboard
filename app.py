...
def one_day_window_bars(tf: str) -> int:
    return {"15m": 96, "1h": 48}.get(tf, 24)  # 15m≈1d, 1h≈2d for stability
...
def build_gate_eval(df_tf: pd.DataFrame, settings: dict) -> Tuple[dict, int, str, int]:
    ...
    def chip(name, enabled_flag, ok, extra=""):
        if not enabled_flag: chips.append(f"{name}–")
        else: chips.append(f"{name}{'✅' if ok else '❌'}{extra}")
...
with expander("Gates"):
    ...
    st.slider("Δ lookback (candles)", 0, 100, int(st.session_state.get("lookback_candles", 3)), 1, key="lookback_candles")
...
for idx, pid in enumerate(pairs):
    if idx % 8 == 0 and idx > 0: time.sleep(0.25)  # gentle throttle
    ...
    if effective_exchange == "Coinbase":
        ...
        if ws_px: last_price = float(ws_px)
    ...
    if st.session_state.get("do_ath", False):
        # lightweight daily window fetch: reuse last 90 days with 1d granularity
        # keeping simple to avoid heavy loops
        hist = get_df(effective_exchange, pid, "1h", limit=24 * 90)  # proxy history
        if hist is not None and len(hist) >= 10:
            hist = hist.copy(); hist.rename(columns={"time": "ts"}, inplace=True)
            aa = ath_atl_info(hist)
            athp, athd, atlp, atld = aa["From ATH %"], aa["ATH date"], aa["From ATL %"], aa["ATL date"]
        else:
            athp = atlp = np.nan; athd = atld = "—"
    else:
        athp = atlp = np.nan; athd = atld = "—"
